...
BrokenLinkChecker.prototype.checkLinks = function() {
  ...
  checker.start(function(err, links) {
    logger.info('Processed '+links.length+' links in '+checker.getSpentTime()+' seconds.');

    // acá debería de guardar los links
  }, function(url) {
  ...
  //   if (error) return logger.error(('[ERROR] Link error.').red);
